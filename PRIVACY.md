...
- 默认情况下，麦克风音频、实时转写上下文和模型回复请求会发送到阿里云
  ...
  该服务。
- 委派给后台 Agent 的任务、必要的对话上下文和任务结果会发送到本机运行或用户
  指定的 OpenCode/OpenClaw。后台 Agent 还可能按照用户配置调用模型、工具、MCP
  服务或访问项目文件；这些服务各自的隐私政策同样适用。
...
本项目无法控制 DashScope、OpenCode、OpenClaw、模型提供商、MCP 服务或用户打开
的第三方链接如何处理数据。正式使用前，请审阅相关服务的隐私条款并选择合适的
部署方式。
