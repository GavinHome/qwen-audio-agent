...
export default function MapPanel({ navState, navProgress, mapActions, routeStrategy, onStrategyChange }) {
  ...
  const progressStageLabel = useMemo(() => {
    if (activeNavProgress?.stage === 'searching_destination' || activeNavProgress?.stage === 'searching_via') return '查找目的地'
    if (activeNavProgress?.stage === 'destination_locked' || activeNavProgress?.stage === 'via_locked') return '目的地已锁定'
  ...
  useEffect(() => {
    processNextRef.current = () => {
      ...
      if (next.action === 'add_marker') {
        ...
        const cls = next.role === 'via' ? 'via-marker' : 'dest-marker'
        const offset = next.role === 'via' ? new AMap.Pixel(-8, -8) : new AMap.Pixel(-10, -10)
      ...
      if (route.polyline) {
        ...
        if (points.length > 0) {
          ...
          scheduleCameraStep(() => {
            ...
            if (route.viaLocation) {
              const [vLng, vLat] = route.viaLocation.split(',').map(Number)
              const viaMarker = new AMap.Marker({
                position: new AMap.LngLat(vLng, vLat),
                map,
                content: '<div class="via-marker marker-pop"></div>',
                offset: new AMap.Pixel(-8, -8),
              })
              markersRef.current.push(viaMarker)
            }
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      if (activeNavProgress.stage === 'searching_destination' || activeNavProgress.stage === 'searching_via') {
        clearCameraTimeline()
        setCameraStage('focus')
        tweenCamera({ center: DEFAULT_CENTER, zoom: 16.2, pitch: 48, rotation: -8 }, 520)
      } else if (activeNavProgress.stage === 'destination_locked' || activeNavProgress.stage === 'via_locked') {
        setCameraStage('destination')
