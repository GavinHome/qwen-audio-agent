...
export default {
  ...
  function: {
    name: 'navigation',
    description: '导航控制，支持开始导航、停止导航、查询路线',
    parameters: {
      type: 'object',
      properties: {
        action: {
          ...
          description: 'start=开始导航, stop=停止导航, query=查询路线',
        },
        destination: {
          type: 'string',
          description: '目的地名称或地址（start/query时必填）',
        },
        via: {
          type: 'string',
          description: '途经点（可选）',
  ...
  execute: async (params, context) => {
    ...
    if (!destination) { clearCallListener(); return { result: '请提供目的地' } }
    ...
    let originLoc = ORIGIN_LOCATION
    let destLoc = destLocation
